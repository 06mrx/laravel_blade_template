    ...
    <div class="email-container">
        ...
        <div class="header">
            @if($configuration->business_logo)
                <img src="{{env('APP_URL').'/storage/'.$configuration->business_logo }}" alt="{{ $configuration->business_logo }}" alt="{{ $configuration->business_name }}" class="logo">
                {{-- <span>{{ $configuration->business_logo }}</span> --}}
            @else
            ...
            <p class="tagline">NEXA Billing</p>
        </div>

        <!-- Konten Utama -->
        <div class="content">
            <div class="icon">⚠️</div>
            <h1>Akun Internet Anda Akan Kedaluwarsa</h1>
            <p>Halo <strong>{{ $customer->name }}</strong>,</p>
            <p>Akun internet Anda Akan <strong>berakhir pada {{ $customer->expired_at }}</strong>.</p>
