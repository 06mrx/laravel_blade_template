...
@section('page-title', 'Edit MikroTik Device')

@section('content')
    <div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('tenant.mikrotik.update', $mikrotik) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $mikrotik->name) }}"
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="active" {{ old('status', $mikrotik->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ old('status', $mikrotik->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $mikrotik->ip_address) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div class="mb-6">
                ...
                <input type="number" name="port" id="port" value="{{ old('port', $mikrotik->port) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div class="mb-6">
                ...
                <input type="text" name="username" id="username" value="{{ old('username', $mikrotik->username) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="email" name="gmail" id="gmail" value="{{ old('gmail', $mikrotik->gmail) }}"
                    ...
                    <div>
                        <label for="app_password" class="block text-xs font-medium text-gray-700 mb-1">App Password (Kosongkan jika tidak ingin mengubah)</label>
                        ...
                        @if($mikrotik->gmail)
                            <p class="text-xs text-green-600 mt-1">Saat ini: <strong>{{ $mikrotik->gmail }}</strong></p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="e.g. MikroTik di lokasi Bandung">{{ old('description', $mikrotik->description) }}</textarea>
                @error('description')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
