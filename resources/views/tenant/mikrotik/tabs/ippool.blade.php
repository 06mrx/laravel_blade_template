<!-- resources/views/tenant/mikrotik/tabs/ippool.blade.php -->

<div class="flex justify-between items-center mb-4">
    <h3 class="text-lg font-medium text-gray-800">IP Pools</h3>
    <a href="{{ route('tenant.ip_pool.create', ['from_mikrotik' => $mikrotik->id]) }}"
...
<table class="min-w-full divide-y divide-gray-200 text-sm">
    <thead class="bg-gray-50">
        <tr>
            ...
            <th class="px-4 py-2 text-left">Next Pool</th>
            <th class="px-4 py-2 text-right">Actions</th>
        </tr>
    </thead>
    <tbody class="bg-white divide-y divide-gray-200">
        @forelse ($mikrotik->ipPools as $pool)
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-2 font-medium">{{ $pool->name }}</td>
                <td class="px-4 py-2">{{ $pool->range }}</td>
                <td class="px-4 py-2">{{ $pool->next_pool ?? '-' }}</td>
                {{-- <td class="px-4 py-2 text-right space-x-2">
                    <a href="{{ route('tenant.ip_pool.edit', ['ip_pool' =>$pool, 'from_mikrotik' => $mikrotik->id]) }}"
                       class="text-blue-600 hover:text-blue-900 text-sm">Edit</a>
                    <button
                        type="button"
                        @click="$dispatch('open-delete-modal', {
                            url: '{{ route('tenant.ip_pool.destroy', ['' => $pool, 'from_mikrotik' => $mikrotik->id]) }}',
                            name: '{{ $pool->name }}',
                            method: 'DELETE',
                ...
                <td class="px-4 py-2 text-right">
                    <div class="flex items-center justify-end space-x-1">
                        ...
                        <button type="button"
                            @click="$dispatch('open-delete-modal', {
                                url: '{{ route('tenant.ip_pool.destroy', ['ip_pool' => $pool, 'from_mikrotik' => $mikrotik->id]) }}',
                                name: '{{ $pool->name }}',
                                method: 'DELETE',
                                from_mikrotik: '{{ $mikrotik->id }}'
                            })"
        ...
        @empty
            <tr>
                <td colspan="4" class="px-4 py-4 text-center text-gray-500">Tidak ada IP Pool ditemukan.</td>
            </tr>
        @endforelse
    </tbody>
</table>