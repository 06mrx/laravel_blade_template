...
@section('page-title', 'Kelola Daftar Odc')

@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-gray-700">Daftar ODC</h2>
        <div class="flex space-x-4 items-center">
            <!-- Form Pencarian -->
            <form action="{{ route('tenant.odc.index') }}" method="GET" class="relative">
                ...
                @if (request('search'))
                    <button type="button" onclick="window.location='{{ route('tenant.odc.index') }}'"
            ...
            <form action="{{ route('tenant.odc.index') }}" method="GET" class="flex items-center space-x-2">
            ...
            <!-- Tombol Add New -->
            <a href="{{ route('tenant.odc.create') }}"
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Perangkat Mikrotik</th>
                ...
                @forelse ($models as $model)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-2 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <a href="{{ route('tenant.odc.show', $model) }}"
                                class="text-yellow-600 hover:text-yellow-900">Odp</a>
                            <a href="{{ route('tenant.odc.edit', $model) }}"
                                class="text-blue-600 hover:text-blue-900">Edit</a>
                            <button type="button"
                                @click="$dispatch('open-delete-modal', {
                                        url: '{{ route('tenant.odc.destroy', $model) }}',
                                        name: '{{ $model->name }}'
                                    })"
