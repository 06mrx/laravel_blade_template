...
@section('page-title', 'Kelola Daftar Odp')

@section('content')
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-semibold text-gray-700">Daftar ODP dari {{ $Odc->name }}</h2>
        <div class="flex space-x-4 items-center">
            <!-- Form Pencarian -->
            <form action="{{ route('tenant.odc.odp.index', $Odc->id) }}" method="GET" class="relative">
                ...
                @if (request('search'))
                    <button type="button" onclick="window.location='{{ route('tenant.odc.odp.index', $Odc->id) }}'"
            ...
            <form action="{{ route('tenant.odc.odp.index', $Odc->id) }}" method="GET" class="flex items-center space-x-2">
            ...
            <!-- Tombol Add New -->
            <a href="{{ route('tenant.odc.odp.create', ['odc' => $Odc->id]) }}"
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($Odc->odps as $key => $model)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-900">
                            {{ $key + 1 }}</td>
                        <td class="px-6 py-2 whitespace-nowrap text-sm font-medium text-gray-900">
                            {{ $model->name }} <br/> <span class="text-xs text-gray-500"> {{$model->name}} </span>
                        </td>                      
                        <td class="px-6 py-2 whitespace-nowrap text-right text-sm font-medium space-x-2">
                           
                            <a href="{{ route('tenant.odc.odp.edit', [$Odc, $model]) }}"
                                class="text-blue-600 hover:text-blue-900">Edit</a>
                            <button type="button"
                                @click="$dispatch('open-delete-modal', {
                                        url: '{{ route('tenant.odc.odp.destroy', [$Odc, $model]) }}',
                                        name: '{{ $model->name }}'
                                    })"
