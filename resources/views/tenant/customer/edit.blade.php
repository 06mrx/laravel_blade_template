...
@section('content')
    <div class="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <form
            action="{{ route('tenant.customer.update', ['customer' => $customer, 'from_mikrotik' => $customer->mikrotik_id]) }}"
            method="POST">
            @csrf
            @method('PUT')
            ...
            <!-- ktp -->
            <div class="mb-6">
                <label for="id_number" class="block text-sm font-medium text-gray-700 mb-1">Nomor KTP / NIK</label>
            ...
            <div class="mb-6">
                ...
                <textarea name="address" id="address" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="e.g. Jl. Merdeka No. 1, Jakarta">{{ old('address', $customer->address) }}</textarea>
                @error('address')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
                ...
                <select name="ip_pool_id" id="ip_pool_id"
                    ...
                    @foreach ($ipPools as $pool)
                        <option value="{{ $pool->id }}"
                            {{ old('ip_pool_id', $customer->ip_pool_id) == $pool->id ? 'selected' : '' }}>
            ...
            <!-- Expired At -->
            <div class="mb-6">
                <label for="expired_at" class="block text-sm font-medium text-gray-700 mb-1">Tgl. Kadaluarsa</label>
                <input type="datetime-local" name="expired_at" id="expired_at"
                    value="{{ old('expired_at', optional($customer->expired_at)->format('Y-m-d\TH:i')) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="mb-6">
                ...
                <select name="status" id="is_active"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="terdaftar" {{ $customer->status == 'terdaftar' ? 'selected' : '' }}>Terdaftar</option>
                    <option value="aktif" {{ $customer->status == 'aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="isolir" {{ $customer->status == 'isolir' ? 'selected' : '' }}>Isolir</option>
                </select>
                @error('is_active')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div class="flex justify-end space-x-3 mt-6">
                <a href="{{ route('tenant.mikrotik.show', $customer->mikrotik_id) }}"
